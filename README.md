...
Coarse pluggable: you can choose the coarse thumbnail format at encode time (`--coarse-format png|jpeg|webp` and optional `--coarse-quality` for lossy formats). Decoder auto-detects via Pillow.
...
Unit: `src/systemd/holo_mesh_node.service`, env template: `src/systemd/holo_mesh_node.env` → `/etc/default/holo_mesh_node`.  
...
- Sender: `src/systemd/holo_mesh_sender.service` + `src/systemd/holo_mesh_sender.env` (set `HXS_URI`, `HXS_CHUNK_DIR`, `HXS_PEER`, `HXS_MAX_PAYLOAD`, optional auth/extra).  
- Receiver: `src/systemd/holo_mesh_receiver.service` + `src/systemd/holo_mesh_receiver.env` (set `HXR_LISTEN`, `HXR_OUT_DIR`, `HXR_DECODE`, `HXR_MAX_PAYLOAD`, optional auth/extra).
...
- `encode_and_corrupt.py` encodes `flower.jpg` (under `src/`), deletes random chunks, and decodes to show graceful degradation (falls back to a generated gradient if missing).
...
What you get:
- `encode_and_corrupt.py` → `src/examples/out/flower.holo` and `flower_recon.png` (or `gradient.*` if the sample is missing).
- `pack_and_extract.py` → `src/examples/out/scene.holo` (packed), plus `src/examples/out/galaxy_recon.png` from the damaged field.
- `heal_demo.py` → `src/examples/out/no-signal_degraded.png`, `no-signal_healed.holo`, `no-signal_healed.png`.
- `mesh_loopback.py` → sender writes `src/examples/out/galaxy.holo`, receiver reconstructs `src/examples/out/galaxy_mesh_recon.png` addressed by `holo://demo/galaxy`.
- `scene_stream_demo.py` → sender writes per-frame containers under `src/examples/out/scene_stream/`, receiver decodes frames under the chosen `--decode-dir` (image + audio) for anytime decode.
...
- Field hygiene: `src/examples/field_tools.py` can list, drop, copy, and merge chunks to keep stores clean or to curate partial fields.
...
Rizzo, A. (2025). *HolographiX: a percept-first codec and network substrate for Large Vision Models and Large Audio Models over UDP (v1.4).* Zenodo. (v1.4). [DOI: 10.5281/zenodo.17914661](https://doi.org/10.5281/zenodo.17914661)
