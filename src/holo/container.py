...
def pack_objects_holo_dir(
    ...
    *,
    ...
    if block_count is None and max_chunk_bytes is None:
    ...
    if block_count is None and max_chunk_bytes is not None:
        coarse_total = sum(len(obj["coarse_comp"]) for obj in encoded)  # type: ignore[index]
        block_count = _select_block_count_bytes(total_residual * 2, coarse_total, max_chunk_bytes)
